...
Write-Host "   aws s3 sync ./dbt/ s3://ons-dg-00-dev-stage/dbt/ --exclude 'target/*' --exclude 'dbt_packages/*'" -ForegroundColor Gray
Write-Host "`n2. Upload da DAG:" -ForegroundColor White
Write-Host "   aws s3 cp ./dags/dbt_athena_example.py s3://ons-dg-00-dev-stage/dags/" -ForegroundColor Gray
